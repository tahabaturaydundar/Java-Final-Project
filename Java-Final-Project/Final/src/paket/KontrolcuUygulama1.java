...
public class KontrolcuUygulama1 implements Initializable {
    ...
    void personelGüncelle(ActionEvent event) {
    	...
    	if (index != -1) {
        	...
        	try {
    			sorgu = baðlantý.prepareStatement("update Tablo set ADI=?,SOYADI=?,TARÝH=?,MAAÞ=?,CÝNSÝYET=?,DEPARTMAN=? where TARÝH ="+"'"+tarih+"'");
    		    sorgu.setString(1, ad);
    		    sorgu.setString(2, soyad);
    		    sorgu.setDate(3, Date.valueOf(tarih));
    		    sorgu.setDouble(4, maaþ);
        	...
        	bilgimesaji.setHeaderText("Baþarý ile güncellendi...");
        	bilgimesaji.show();
		}
    	else {
			hatamesaji.setTitle("Hata");
			hatamesaji.setHeaderText("Güncellenecek kayýt seçilmedi...");
			hatamesaji.show();
		}
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		tblPersonel.getSelectionModel().selectedIndexProperty().addListener(new ChangeListener<Number>() {
			...
			public void changed(ObservableValue<? extends Number> observable, Number oldValue, Number newValue) {
				...
				if (index >-1) {
					Personel seçilenpersonel = personeller.get(index);
					spnMaaþ.getValueFactory().setValue(seçilenpersonel.getMaaþ());
					txtAdý.setText(seçilenpersonel.getAd());
					txtSoyadý.setText(seçilenpersonel.getSoyad());
					cmbCinsiyet.setValue(seçilenpersonel.getCinsiyet());
					cmbDepartman.setValue(seçilenpersonel.getDepartman());
					dpTarih.setValue(seçilenpersonel.getTarih());
